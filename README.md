...
![изображение](https://github.com/JuFnd/highload-hw-slack/assets/109366718/6290bf04-e3dd-4158-8834-d63a14de65fb)
...
|Reaction|Postgresql|
|User|Postgresql|
|MessageFile|Postgresql|
|MessageChecked|Postgresql|
...
Можно индексировать следующие столбцы для ускорения запросов:
  ...
  - Таблица "Канал" (Channel): id, channel_id, creator_id
  - Таблица "Сообщения" (Messages): sender_id, message_file_id
  - Таблица "Треды" (Threads): id, parent_message_id
  - Таблица "Файлы" (Files): id, creator_id
  - Таблица "Реакции" (Reactions): user_id, message_id
  - Таблица "Пользователи" (Users): id, username
  - Таблица "Пароли" (Passwords): id
  - Таблица "Членство в каналах" (ChannelMemberships): channel_id
  - Таблица "Проверенные сообщения" (MessageChecked): user_id, message_id
  - Таблица "Файлы сообщений" (MessageFiles): file_id, message_id
...
Шардинг можно провести по следующим критериям:
  ...
  - По id сообщения (Messages) - Для сообщений можно провести горизонтальный шардинг либо на основе уникального идентификатора, либо на основе времени отправки сообщения (например, разделив сообщения на месяцы или недели). Это позволит распределить данные по нескольким шардам в зависимости от объёма сообщений, улучшая масштабируемость и производительность.
...
VRRP (Virtual Router Redundancy Protocol) - это протокол, который позволяет создавать виртуальный IP-адрес и виртуальный маршрутизатор, который будет обслуживаться несколькими физическими маршрутизаторами. Один из маршрутизаторов выбирается в качестве мастера (master), который обрабатывает трафик, а остальные маршрутизаторы работают в режиме резервного (backup). Если мастер-маршрутизатор выходит из строя, один из резервных маршрутизаторов автоматически становится мастером и продолжает обслуживать трафик. Это обеспечивает непрерывность работы сети и предотвращает ее простой в случае отказа одного из маршрутизаторов.

CARP (Common Address Redundancy Protocol) - это протокол, разработанный для обеспечения высокой доступности IP-адресов. Он позволяет нескольким устройствам с одним и тем же IP-адресом работать в режиме резервного копирования. Один из устройств выбирается в качестве мастера, который обрабатывает трафик, а остальные устройства работают в режиме резервного копирования. Если мастер-устройство выходит из строя, одно из резервных устройств автоматически становится мастером и продолжает обслуживать трафик. CARP обеспечивает непрерывность работы сети и позволяет быстро восстановить работу в случае отказа мастер-устройства.

Envoy - это современный, высокопроизводительный и компактный прокси-сервер, используемый в качестве краевого (edge) и сервисного (service) прокси. Был выбран т.к поддерживает большое кол-во протоколов таких как gRPC или WebSocket. 
### Источники
