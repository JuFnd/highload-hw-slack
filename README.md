...
![image](https://github.com/JuFnd/highload-hw-slack/assets/109366718/d679dacd-fd87-4b55-8b03-7a0dbd7059e5)


  #### Table Channels: 
  Эта таблица содержит информацию о каналах общения. Каждый канал имеет уникальный идентификатор (id), название (name), тип (type), описание (description), и идентификатор создателя (creator_id), который ссылается на таблицу Users.

  #### Table Messages: 
  Эта таблица хранит информацию о сообщениях. Каждое сообщение имеет уникальный идентификатор (id), идентификатор отправителя (sender_id), идентификатор родительского сообщения (parent_message_id), временную метку (timestamp), содержание (content), идентификатор файла сообщения (message_file_id), и реакцию в виде эмодзи (reaction_emoji).

  #### Table Threads: 
  Эта таблица содержит информацию о темах обсуждения. Каждая тема имеет уникальный идентификатор (id), название (name), и идентификатор родительского сообщения (parent_message_id).

  #### Table Files: 
  Эта таблица хранит информацию о файлах, прикрепленных к сообщениям. Каждый файл имеет уникальный идентификатор (id), размер (size), путь к файлу (path), и идентификатор создателя (creator_id).

  #### Table Reactions: 
  Эта таблица содержит информацию о реакциях пользователей на сообщения. Каждая реакция имеет уникальный идентификатор (id), эмодзи (emoji), идентификатор пользователя (user_id), и идентификатор сообщения (message_id).

  #### Table Message_File: 
  Эта таблица представляет связь между сообщениями и файлами. Каждая запись имеет уникальный идентификатор (id), идентификатор сообщения (message_id), и идентификатор файла (file_id).

  #### Table Users: 
  Эта таблица содержит информацию о пользователях. Каждый пользователь имеет уникальный идентификатор (id), имя пользователя (username), электронную почту (email), часовой пояс (timezone), аватар (avatar), и время создания (created_at).

  #### Table Message_Channel: 
  Эта таблица представляет связь между сообщениями и каналами. Каждая запись имеет уникальный идентификатор (id), идентификатор сообщения (message_id), и идентификатор канала (channel_id).

  #### Table Message_Thread: 
  Эта таблица представляет связь между сообщениями и темами. Каждая запись имеет уникальный идентификатор (id), идентификатор сообщения (message_id), и идентификатор темы (thread_id).

  #### Table Message_User: 
  Эта таблица представляет связь между сообщениями и пользователями. Каждая запись имеет уникальный идентификатор (id), идентификатор пользователя (user_id), и идентификатор сообщения (message_id).
