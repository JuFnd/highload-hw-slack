...
### Шардинг
Ring-based algorithms (кольцевые алгоритмы) — это класс алгоритмов, используемых для разделения данных между узлами в распределённых системах. Основная идея состоит в том, что узлы (или шарды) располагаются на виртуальном кольце, а ключи данных hashируются в точки на этом кольце, после чего данные сопоставляются с ближайшим узлом (шардом) по часовой стрелке.
