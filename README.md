...
|Создание чатов|37.92 млн|345.6|
|Создание каналов|46.92 млн|458.3|
|Создание тредов|34.92 млн|404.1|
|Реакции|14.92 млн|114.1|
...
- Использование механизма балансировки нагрузки, такого как GSLB, для распределения входящего трафика по географическим критериям. Эти системы используют DNS-записи для связывания доменных имён с конкретными IP-адресами, связанными с индивидуальными дата-центрами.
...
Для обеспечения высокой доступности используется VRRP на балансировщиках, а служба keepalived отслеживает состояние L7 балансировщика и уведомляет об этом L3 балансировщик.
#### L7 Балансировщик:
...
Хранит информацию о пользователях системы. Каждый пользователь имеет уникальный идентификатор (id), имя пользователя (username), адрес электронной почты (email), часовой пояс (timezone), аватар (avatar), дату регистрации (created_at) и  в зашифрованном виде пароль (password).
...
![изображение](https://github.com/JuFnd/highload-hw-slack/assets/109366718/02e2e193-84d5-4946-8504-8cb9c15bd12e)
...
##### Допускается потеря части функционала:
   - Поиск по сообщения: В случае отказа сервиса поиска, перенаправить запросы на поиск на прямую в бд с сообщениями, в случае если это возможно.
   - Реакции на сообщения: В случае перегрузки сервиса сообщений, отключить сервис реакций.
